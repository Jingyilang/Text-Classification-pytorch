...
class CNN(nn.Module):
    def __init__(self, **kwargs):
        ...
        for i in range(len(self.FILTERS)):
            ...
            setattr(self, 'conv_{i}'.format(i=i), conv)

        # simple method
        # self.conv_1 = nn.Conv1d(self.IN_CHANNEL, self.FILTER_NUM[1], self.WORD_DIM * self.FILTERS[1], stride=self.WORD_DIM)
        # self.conv_2 = nn.Conv1d(self.IN_CHANNEL, self.FILTER_NUM[2], self.WORD_DIM * self.FILTERS[2], stride=self.WORD_DIM)
        # self.conv_3 = nn.Conv1d(self.IN_CHANNEL, self.FILTER_NUM[3], self.WORD_DIM * self.FILTERS[3], stride=self.WORD_DIM)

        self.fc = nn.Linear(sum(self.FILTER_NUM), self.CLASS_SIZE)

    def get_conv(self, i):
        return getattr(self, 'conv_{i}'.format(i=i))
