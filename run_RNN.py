...
def test(data, model, params, mode="test"):
    ...
    for i in range(0, len(x), params["BATCH_SIZE"]):
        ...
        correct = sum([1 if p == y else 0 for p, y in zip(pred, batch_y)])
        corrects += correct

    acc = float(corrects) / len(y)

    return acc


def main():
    parser = argparse.ArgumentParser(description="-----Text classification by BiGRU-----")
    parser.add_argument("--mode", default="train", help="train: train (with test) a model / test: test/dev")
    parser.add_argument("--model", default="rand", help="available models:rand, static, non-static")
